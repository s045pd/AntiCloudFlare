...
def Crack(Resp=None, Content=None, Url=None):
    ...
    JSCode = jq(Content)("script").eq(0).text().replace("\n", "").split("setTimeout(function(){")[1].split(
        "f.action += location.hash")[0].replace("a.value", "res").replace("t.length", "12").split(";")
    ...
    Params["jschl_answer"] = execjs.eval(
        JSCode[-3].replace(Key3, str(StartData[Key2])))
...
if __name__ == '__main__':
    url = 'https://coinone.co.kr/landing/main'
    print(Crack(Content=requests.get(url).text,Url=url))
